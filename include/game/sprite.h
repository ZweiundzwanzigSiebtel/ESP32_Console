...
class Sprite {
   public:
    Sprite(
        gsl::span<const uint16_t> pixel_data, uint16_t x_position, uint16_t y_position, uint16_t z_value, uint16_t width, uint16_t height, uint16_t transparent_pixel);
